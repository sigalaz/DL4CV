...
def reverse_rows(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    idx = torch.arange(-x.shape[0], 0).__reversed__()
    y = x[idx, :]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def count_negative_entries(x: Tensor) -> Tensor:
    """
...
def make_one_hot(x: List[int]) -> Tensor:
    ...
    y = torch.zeros([len(x), max(x)+1], dtype=float32)

    y[torch.arange(len(x)), x] = 1

    # TODO: think a way to make one hot encoding using boolean tensor indexing
...
def reshape_practice(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    y = x.reshape(2, 3, 4).transpose(1, 2).reshape(8, 3).transpose(0, 1)
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    y = torch.clone(x)
    y[range(0, len(row_min_vals)), row_min_idxs] = 0
...
def batched_matrix_multiply_loop(x: Tensor, y: Tensor) -> Tensor:
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    z = torch.ones((x.shape[0], x.shape[1], y.shape[2]))
    for batch in torch.arange(x.shape[0]):
        z[batch] = torch.mm(x[batch], y[batch])
