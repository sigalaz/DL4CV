...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for train_id, train_img in enumerate(x_train_flattenned):
        for test_id, test_img in enumerate(x_test_flattened):
            diff = torch.sqrt(torch.sum((train_img - test_img)**2))
            dists[train_id, test_id] = diff
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    dists = dists.t()
    #print(f'transposed version: \n {dists}')
    k_values, indices = torch.topk(dists, k, dim=1, largest=False)
    #print(f'\nk_values \n{k_values}\n')
    #print(f'\nk_indices \n{indices}\n')

    k_nearest_labels = y_train[indices]

    #print(f'most near label : \n{k_nearest_labels}\n')
    #print(f'y_train labels \n{y_train}\n')

    #sum_unique_labels = torch.unique(k_nearest_labels, dim=0)

    # TODO : I think I don't need a for loop to check for unique values and sum them. torch.unique would be enough for that end
    for i, k_nearest_labels in enumerate(k_nearest_labels):
    ...
    #print(f'sum of unique labels on each row: \n{sum_unique_labels}')
    # for i, label in enumerate(y_train):
    #    print(k_nearest_labels[])

    #values, count = torch.unique(k_values, dim=0, return_counts=True)
